...
function Start ()
{
	PlayerPrefs.SetInt("HardcoreAchiv", 0);
	gameEnded = false;
...
function Update ()
{
	PlayerPrefs.SetInt("HardcoreAchiv", score);
	if(lifePlus >= 100)
	{
		lifePlus -= 100;
	...
	//	pP = 0;
	//}
	highScoreGO.GetComponent.<TextMesh>().text = "Best Score : " + PlayerPrefs.GetInt("hardcoreHighScore");
	if(gameEnded == false)
	{
		scoreGO.GetComponent.<TextMesh>().text = "" + score;
	}
	if(score > PlayerPrefs.GetInt("hardcoreHighScore"))
	{
		PlayerPrefs.SetInt("hardcoreHighScore", score);
	}
